...
function getEventBody (type, options) {
  ...
  const 
    ...
    mid = options.mid || "mid.$cAAJsujCd2ORnUSd1SFhH-IsSityC",
    title = options.title || "TITLE",
    payload = options.payload || "PAYLOAD",
    referral_param = options.referral_param || "REFERRAL_PARAM",
    watermark = options.watermark || Math.floor(Math.random() * 900000000000 + 1000000000000),
    text = options.text || "TEXT",
    ad_id = options.ad_id || "AD_ID",
    metadata = options.metadata || "METADATA",
    seq = options.seq || Math.floor(Math.random() * 100 + 10),
    ...
    attachments = [{
      ...
      'url': options.url || 'https://github.com/amuramoto/messenger-test-framework/raw/master/assets/allie.jpg'
    }],
    amount = {
  ...
  switch (type) {
    ...
    case 'messaging_checkout_updates': 
      ...
      eventBody.checkout_update.shipping_address['id'] = 10105655000959552;
      break;
...
function emitEvent (webhook_event) {
  this.webhook.emit(webhook_event);
}

function postEvent (webhook_event) {
...
function Dispatcher (webhook) {
  this.webhook = webhook;
  if (webhook.constructor.name === 'Webhook') {
    this.send = emitEvent.bind(this.webhook);
